...
function JwtTokenHandler(options) {
  var self = this;
  ...
  if (options.scopes) {
    this._requiredScopes = _.isArray(options.scopes) ? options.scopes : [options.scopes]
  };
...
Object.defineProperty(JwtTokenHandler.prototype, 'requiredScopes', {
  get: function() {
...
JwtTokenHandler.prototype.verifyToken = function(token, options, cb) {
  ...
  const requiredScopes = _.union(options.scopes, this._requiredScopes);
      ...
      function(err, claims) {
        ...
        claims.hasScopes = function(scopes) {
          if (scopes) {
            ...
            var grantedScopes = _.intersectionBy(claims[scopeClaimName], scopes);
        ...
        if (requiredScopes && !claims.hasScopes(requiredScopes)) {
          logger('Token does not have required scopes (token=%s expected=%s)',
            claims[scopeClaimName] ? claims[scopeClaimName].join(' ') : '', requiredScopes.join(' '));
          return cb(new BearerTokenError({
            realm: realm,
